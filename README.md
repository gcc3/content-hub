...
there, so `setup.sh` gets it before the build. With no `PAGES_REPO` there is  
nothing to fetch, so `pull.sh` leaves the folder alone and carries on. A  
`src/pages` that is on disk but is not a clone stops `pull.sh` itself: a folder  
that cannot be pulled builds as well as one that can, and ships the pages it  
already held without saying so. Page edits are committed and pushed in that  
repo, not this one.  

Landing pages are optional. Nothing imports `src/pages` by path — the readers  
go through the `@pages` alias, which `src/build/pages-dir.js` points at  
`src/pages-fallback/` when the folder is not there. That fallback holds the  
same three modules with no pages in them, so a deployment without a pages repo  
builds and serves the content app on its own: one prerendered front page, one  
URL in the sitemap, every path answered by the app.  
